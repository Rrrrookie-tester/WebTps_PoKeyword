...
class TestRegistration:
    ...
    @allure.story("rigid and nonrigid")
    def registration(self, registration_data):
        """
        Rigid and nonrigid registration is performed for two series respectively
        ...
        series_num = int(registration_data[2])
    ...
    @allure.story("test rigid and nonrigid")
    @pytest.mark.parametrize('registration_config', registration_config)
    def test_registration(self, registration_config):
        result = self.registration(registration_config)
        assert result, self.logger.error("case: rigid、nonrigid registration failed !")


# if __name__ == '__main__':
#     # pytest.main(['-s', 'test_registration.py'])
#     pytest.main(["-s", "test_registration.py", "--alluredir=../result/test_registration"])
